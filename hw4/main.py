...
def main():
    """
    Quadratic programming minimizes:
    1/2 x^T Q x - c^T x subject to Ax < b and Ex = d

    In the SVM problem formulation:
    x is a vector of alphas with slack variables (xis) appended on the end (2 * n x 1)
    Q is a matrix of dot products between pairs of training examples (symmetric)
    (but also need to append zero matrices to zero out the xi terms in obj. function)

    Use the inequality constraint to enforce alphas and xis are all > 0
    Equality constriant ensures:
    - summation of alpha coefficients x corresponding training label = 0
    - alphas are < regularization term (C) (by setting alpha + xi = C)
    """
    ...
    # draw(train_data, test_data, kernel_func=linear_kernel, lambda_param=lambda_param)
